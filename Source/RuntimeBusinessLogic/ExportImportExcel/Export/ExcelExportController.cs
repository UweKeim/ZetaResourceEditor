namespace ZetaResourceEditor.RuntimeBusinessLogic.ExportImportExcel.Export
{
	...
	public class ExcelExportController
	{
		...
		private class PreparedInformation
		{
			...
			//public string[] ReferenceAndDestinationLanguageCodes
			//{
			//    get
			//    {
			//        var result = new List<string>();

			//        if (!string.IsNullOrEmpty(ReferenceLanguageCode))
			//        {
			//            result.Add(ReferenceLanguageCode);
			//        }

			//        if (DestinationLanguageCodes != null && DestinationLanguageCodes.Length > 0)
			//        {
			//            result.AddRange(DestinationLanguageCodes);
			//        }
			...
			public bool ExportCompletelyEmptyRows
			{
				get;
				set;
			}

			public bool OnlyExportRowsWithChangedTexts
			{
				get;
				set;
			}

			public bool ExportAllGroups
			{
				get;
				set;
			}

			public bool ExportFileGroupColumn
			{
				get;
				set;
			}

			public bool ExportNameColumn
			{
				get;
				set;
			}

			public bool ExportCommentColumn
			{
				get;
				set;
			}
			...
			ICollection<string> savedFiles)
		{
			...
			foreach (var fileGroup in preparedInformation.FileGroups)
			{
				...
				else
				{
					worksheetIndex = workbook.Worksheets.Add();
					worksheet = workbook.Worksheets[worksheetIndex];
					worksheet.Name = generateWorksheetName(preparedInformation, fileGroup, preparedInformation.Project);
				}
				...
				if (wantNewSheet)
				{
					...
					if (preparedInformation.ExportReferenceLanguageColumn)
					{
						// Reference language.
						headerCell = cells[headerStartRowIndex, columnStartIndex + offset];
						headerCell.PutValue(preparedInformation.ReferenceLanguageCode);
						makeHeaderCell(headerCell);
						offset++;
					}
					...
					if (preparedInformation.ExportCommentColumn)
					{
						// String name.
						headerCell = cells[headerStartRowIndex, columnStartIndex + offset + realRunningColumnIndex];
						headerCell.PutValue(Resources.SR_CommandProcessorSend_Process_Comment);
						makeHeaderCell(headerCell);
						/*offset++;*/
					}
				...
				foreach (DataRow row in table.Rows)
				{
						...
						CommentVisibilityScope.InMemory))
					{
							...
							row))
						{
							...
							if (preparedInformation.ExportFileGroupColumn)
							{
								...
								contentCell.PutValue(fileGroup.GetChecksum(preparedInformation.Project));
							...
							{
								// String name.
								var contentCell = cells[currentRowIndex, columnStartIndex + offset];
								contentCell.PutValue(row[1]); // Column 0=FileGroup checksum, column 1=Tag name.
								makeCellReadOnly(contentCell);
								...
								++sourceColumnIndex)
							{
								...
								{
									var contentCell =
										cells[currentRowIndex, (columnStartIndex - 1) + effectiveDestinationColumnIndex + offset];
							...
							if (preparedInformation.ExportCommentColumn)
							{
								...
								contentCell.PutValue(DataProcessing.GetComment(preparedInformation.Project, row));
					...
					suggestedWorkSheetNamesToRemove.Remove(worksheet.Name);
						...
						!preparedInformation.ExportAllGroups)
					{
						...
						{
							suggestedWorkSheetNamesToRemove.Add(worksheet.Name);
						}
					}
			...
			if (workbook.Worksheets.Count > 0)
			{
				...
				workbook.Save(preparedInformation.DestinationFilePath);
			}
			else
			{
				bw.ReportProgress(
