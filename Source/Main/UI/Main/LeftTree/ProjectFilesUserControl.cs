	...
	UserControlBase
{
	public ProjectFilesUserControl()
	{
		...
		treeView.SelectImageList = ImageCollectionHelper.IcSvg;
	...
	public void RemoveResourceFilesWithDialog()
	{
		...
		if (dr == DialogResult.Yes)
		{
			...
			while (node is { Tag: not FileGroup })
			{
	...
	public bool CanAddResourceFilesToProject => Project != null && Project != Project.Empty &&
												treeView.SelectedNode is
												{
													Tag: RuntimeBusinessLogic.Projects.Project
													or ProjectFolder
												};
	...
	public bool CanCreateNewFiles => Project != null && Project != Project.Empty &&
									 treeView.SelectedNode is
									 {
										 Tag: RuntimeBusinessLogic.Projects.Project or
										 ProjectFolder
									 };
		...
		TreeListNode dragNode)
	{
		...
		{
			if (dragNode.Tag is FileGroup or ProjectFolder)
			{
				return targetNode.Tag is ProjectFolder or RuntimeBusinessLogic.Projects.Project ? DragDropEffects.Move : DragDropEffects.None;
			}
			else
			{
