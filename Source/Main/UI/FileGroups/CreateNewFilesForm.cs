	...
	FormBase
{
	...
	protected override void InitiallyFillLists()
	{
			...
			if (!string.IsNullOrEmpty(languageCode.Item1))
			{
				referenceLanguageComboBox.Properties.Items.Add(
					new MyTuple<string, MyTuple<string, string>>(
						$@"{(plcl.Contains(languageCode.Item1) ? @"* " : string.Empty)}{LanguageCodeDetection.MakeValidCulture(languageCode.Item1).DisplayName} ({languageCode})",
						languageCode));
			}
		}
		...
		// ReSharper restore LoopCanBeConvertedToQuery
		{
			items.Add(
				new(
					$@"{(plcl.Contains(culture.Name.ToLowerInvariant()) ? @"* " : string.Empty)}{culture.DisplayName} [{culture.Name}]",
					culture));
		}
